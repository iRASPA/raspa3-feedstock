...
if [[ "${CONDA_BUILD_CROSS_COMPILATION:-}" != "1" ]]; then
  ...
  if [[ "${target_platform}" == linux-aarch64 ]]; then 
    ...
    cmake -B build --preset=linux_conda_raspa3 ${CMAKE_ARGS} -DCMAKE_CXX_COMPILER_AR="$BUILD_PREFIX/bin/powerpc64le-conda_cos7-linux-gnu-ar" -DCMAKE_CXX_COMPILER_RANLIB="$BUILD_PREFIX/bin/powerpc64le-conda_cos7-linux-gnu-ranlib" -DCMAKE_POLICY_VERSION_MINIMUM=3.32
  elif  [[ "${target_platform}" == osx-* ]]; then
